...
export class CreateTestHeaderComponent implements OnInit {
  ...
  showEditButton(testName: string) {
    this.isLabelVisible = true;
    if (this.editedTestName) this.nameOfTest = this.editedTestName;
    else this.nameOfTest = this.testNameReference;
    if (
      testName === "" ||
      !testName.match(RegExp("^[a-zA-Z0-9_ @]*$")) ||
      this.isTestNameExist === true
    ) {
      if (this.nameOfTest) this.testDetails.testName = this.nameOfTest;
      testName = this.testDetails.testName;
      this.isTestNameExist = false;
    } else if (this.editedTestName)
      this.testDetails.testName = this.editedTestName;
    else this.testDetails.testName = this.testNameReference;
  }
  ...
  updateTestName(id: number, testObject: Test) {
    ...
    if (this.testNameRef) {
      this.testService
        .IsTestNameUnique(this.testNameRef, id)
        .subscribe((isTestNameUnique) => {
          this.isButtonClicked = false;
          if (isTestNameUnique) {
            this.isButtonClicked = true;
    ...
    } else this.isWhiteSpaceError = true;
  }
